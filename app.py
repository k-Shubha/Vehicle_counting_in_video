...
class App(QMainWindow,Ui_mainWindow):
    def __init__(self):
        ...
        data_config = "config/obj.data"
        weights_path = "weights/yolov3-obj_4000.weights"
        model_def = "config/yolov3-obj.cfg"
    ...
    def update_counter_results(self,counter_results):
        ...
        df = pd.read_csv("./results/results.txt", sep=" ", header=None)
        df.to_excel('./results/output.xlsx')
		#df.reset_index(drop=True, inplace=True)
        df3= df.groupby(df.columns[2]).size().reset_index(name='count') 
        df3.columns = ['Type of vehicle', 'Count']
        df3.to_excel('./results/output.xlsx', index=False)
